...
import type { Palette, Role } from "./akmon";
...
export function canvasToTailwind(doc: CvDoc, pal: Palette): string {
  const get = (r: Role) => pal.swatches.find((s) => s.role === r)?.hex ?? "#888888";
  const bg = get("background");
  const surf = get("surface");
  const tx = get("text");
  const mut = get("muted");
  ...
  if (!page) return "// No pages in canvas";

  return `// Generated by HEPHAESTUS Studio
// Palette: ${pal.name} (${pal.mode} mode)
// Framework: React + Tailwind CSS

import React from 'react';

export default function ${doc.brand.replace(/[^a-zA-Z0-9]/g, "") || "LandingPage"}() {
  return (
    <div className="min-h-screen w-full relative" style={{ backgroundColor: '${bg}', color: '${tx}' }}>
      {/* Top Navbar */}
      <nav className="w-full h-16 border-b flex items-center justify-between px-8" style={{ borderColor: '${border}', backgroundColor: '${surf}' }}>
        <div className="font-bold text-xl tracking-tight" style={{ color: '${tx}' }}>
          ${doc.brand || "Brand"}
        </div>
        <div className="flex items-center gap-6">
          <a href="#features" className="text-sm font-medium hover:opacity-80 transition-opacity" style={{ color: '${mut}' }}>Features</a>
          <a href="#pricing" className="text-sm font-medium hover:opacity-80 transition-opacity" style={{ color: '${mut}' }}>Pricing</a>
          <button className="px-4 py-2 rounded-lg text-sm font-semibold transition-transform active:scale-95" style={{ backgroundColor: '${acc}', color: '${tx}' }}>
            Get Started
          </button>
        </div>
      </nav>
      ...
      <main className="max-w-6xl mx-auto px-6 py-16">
        <div className="relative w-full" style={{ minHeight: '${page.blocks.reduce((m, b) => Math.max(m, b.y + b.h), 800)}px' }}>
${page.blocks
  .map((b) => {
    if (b.kind === "heading") {
      return `          <h2 className="absolute font-bold tracking-tight" style={{ left: '${b.x}px', top: '${b.y}px', width: '${b.w}px', fontSize: '${b.size ?? 36}px', color: '${b.fg ?? tx}' }}>
            ${b.text}
          </h2>`;
    }
    if (b.kind === "text") {
      return `          <p className="absolute leading-relaxed" style={{ left: '${b.x}px', top: '${b.y}px', width: '${b.w}px', color: '${b.fg ?? mut}', fontSize: '${b.size ?? 16}px' }}>
            ${b.text}
          </p>`;
    }
    if (b.kind === "button") {
      return `          <div className="absolute" style={{ left: '${b.x}px', top: '${b.y}px', width: '${b.w}px', height: '${b.h}px' }}>
            <button className="w-full h-full rounded-xl font-semibold shadow-sm transition-transform active:scale-95" style={{ backgroundColor: '${b.bg ?? acc}', color: '${b.fg ?? tx}' }}>
              ${b.text}
            </button>
          </div>`;
    }
    if (b.kind === "card") {
      return `          <div className="absolute p-6 rounded-2xl border flex flex-col justify-between shadow-sm backdrop-blur-sm" style={{ left: '${b.x}px', top: '${b.y}px', width: '${b.w}px', height: '${b.h}px', backgroundColor: '${b.bg ?? surf}', borderColor: '${b.line ?? border}' }}>
            <h3 className="font-bold text-lg" style={{ color: '${b.fg ?? tx}' }}>${b.text}</h3>
            <p className="text-sm leading-relaxed" style={{ color: '${mut}' }}>${b.sub ?? ""}</p>
          </div>`;
    }
    if (b.kind === "stat") {
      return `          <div className="absolute p-6 rounded-2xl border flex flex-col justify-center shadow-sm" style={{ left: '${b.x}px', top: '${b.y}px', width: '${b.w}px', height: '${b.h}px', backgroundColor: '${b.bg ?? surf}', borderColor: '${b.line ?? border}' }}>
            <div className="text-3xl font-extrabold tracking-tight" style={{ color: '${b.fg ?? acc}' }}>${b.text}</div>
            <div className="text-xs uppercase tracking-wider font-semibold mt-1" style={{ color: '${mut}' }}>${b.sub ?? ""}</div>
          </div>`;
    }
    if (b.kind === "chip") {
      return `          <div className="absolute inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border" style={{ left: '${b.x}px', top: '${b.y}px', backgroundColor: '${b.bg ?? surf}', color: '${b.fg ?? acc}', borderColor: '${border}' }}>
            ${b.text}
          </div>`;
    }
    return `          {/* Block ${b.kind} (${b.w}x${b.h}) */}`;
  })
  .join("\n")}
        </div>
      </main>
      ...
      {/* Footer */}
      <footer className="w-full py-12 border-t text-center text-xs" style={{ borderColor: '${border}', color: '${mut}' }}>
        © {new Date().getFullYear()} ${doc.brand || "HEPHAESTUS"}. All rights reserved.
      </footer>
    </div>
  );
}
