...
That's the release gate: 12/12, ~260 assertions, and `run-all.mjs` exits non-zero if any of
them complains. `npm run typecheck` (`tsc -b`, not `tsc --noEmit` — the root config is a
...
The two generated things on the site are regenerable from the repo, so nobody has to open an
image editor to update it: `python3 tools/pixel-art.py` (the hero's pixel bust, the og card,
and the OS icons as inline SVG) and `node tools/shots.mjs` (every screenshot, driven through
the running app).
...
so **publishing a release updates the site without a redeploy** — and it never writes a link
that can 404: while the repo isn't confirmed, the buttons land on "the code" further down the
same page. `node tools/connect.mjs <owner> --check` asks GitHub what the page will see before
you commit anything. `node tools/site-preview.mjs` builds a single-file offline copy for
viewing where network calls are blocked. Full notes, including the two ways to wire up
Cloudflare Pages: [`site/README.md`](site/README.md).

Two things on the site are generated by the repo rather than placed by hand — the pixel-art
bust in the hero (`python3 tools/pixel-art.py`: a grid, a seven-tone ramp, a 4×4 Bayer
dither) and the screenshots (`node tools/shots.mjs`: it drives the running app and shoots each
screen). Both write into `site/assets/`, so the page can be refreshed without opening an image
editor, and nothing about it is a mockup.
